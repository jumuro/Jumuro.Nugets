...
module.exports = function (grunt) {
    ...
    var createConfig = function (environment) {
        var target = 'ErrorHandling_' + environment;
        grunt.config(['replace', target], {
            options: {
                patterns: [{
                    json: grunt.file.readJSON('./appNugets/Espa.Angular.errorhandling/environments/' + environment + '.json')
                }],
            },
            files: [{
                flatten: true,
                src: ['./appNugets/Espa.Angular.errorhandling/Constants/gruntReplaceSource.js'],
                dest: './appNugets/Espa.Angular.errorhandling/Constants/appConfigConstants.js'
            }]
        });
    ...
    var createTfsTask = function (tfs) {
        ...
        var target = tfs + '_ErrorHandlingConfig'

        var command = '"C:\\Program Files (x86)\\Microsoft Visual Studio 12.0\\Common7\\IDE\\tf.exe" ' + tfs + ' ' +
                      config.project.tfsWorkSpace + 'appnugets\\espa.angular.errorhandling\\constants\\appConfigConstants.js';
