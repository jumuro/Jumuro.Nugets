...
module.exports = function (grunt) {
    ...
    var createConfig = function (environment) {
        var target = 'oAuthLogin' + environment;
        grunt.config(['replace', target], {
            options: {
                patterns: [{
                    json: grunt.file.readJSON('./appNugets/Jumuro.Angular.OAuth/environments/' + environment + '.json')
                }],
            },
            files: [{
                flatten: true,
                src: ['./appNugets/Jumuro.Angular.OAuth/Constants/gruntReplaceSource.js'],
                dest: './appNugets/Jumuro.Angular.OAuth/Constants/appConfigConstants.js'
            }]
        });
    ...
    var createTfsTask = function (tfs) {
        ...
        var target = tfs + '_oAuthLogin'

        var command = '"C:\\Program Files (x86)\\Microsoft Visual Studio 12.0\\Common7\\IDE\\tf.exe" ' + tfs + ' ' +
                      config.project.tfsWorkSpace + 'appnugets\\jumuro.angular.OAuth\\constants\\appConfigConstants.js';
